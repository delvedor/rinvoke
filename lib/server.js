...
function Server (opts) {
  ...
  this.register('__procedures__', onProcedures.bind(this))
  function onProcedures (req, reply) {
    reply(null, Object.keys(this._functions).filter(p => p !== '__procedures__'))
  }
}
...
Server.prototype.listen = function (portOrPath, address, cb) {
  ...
  this.ready(err => {
    ...
    if (address) {
      this._server.listen(portOrPath, address, err => {
        cb(err)
        this.emit('listening')
      })
    } else {
